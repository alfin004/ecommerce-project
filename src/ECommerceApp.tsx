...
/* Updated component implementing:
   1) combo discount logic,
   2) centered shop info modal,
   3) responsive grid (1 on mobile, multiple on larger screens),
   4) shop details removed from top and shown inside product descriptions; shop modal remains the only place for full details.
*/
...
function applyDiscountsPerUnit(item: any, quantity: number) {
  ...
  const afterItemDiscount = item.Rate - (item.Rate * (item.Discount || 0) / 100);
  // If combo applies, apply combo discount on top of item-discounted price
  if (item.combo_quantity && item.combo_quantity > 0 && quantity >= item.combo_quantity) {
    const afterCombo = afterItemDiscount - (afterItemDiscount * (item.combo_discount || 0) / 100);
    return afterCombo;
  }
...
export default function ECommerceApp() {
  ...
  const addToCart = (product: any, e?: React.MouseEvent) => {
    ...
    setTimeout(() => setShowCartNotification(false), 1600);
  };
  ...
  const cartTotal = cart.reduce((total, item) => {
    const unitPrice = applyDiscountsPerUnit(item, item.quantity || 1);
  ...
  const handleWhatsAppOrder = () => {
    ...
    cart.forEach((item: any, idx: number) => {
      const unit = applyDiscountsPerUnit(item, item.quantity || 1);
      message += `\n${idx + 1}. ${item.Name}\n   Qty: ${item.quantity}\n   ₹${unit.toFixed(2)} x ${item.quantity} = ₹${(unit * item.quantity).toFixed(2)}\n`;
      if ((item.Discount || 0) > 0 || (item.combo_quantity && item.quantity >= item.combo_quantity)) {
                  ...
                  <div className="space-y-1 text-sm text-gray-600">
                    {cart.map(it => (
                      <div key={it.id} className="flex justify-between">
                        ...
                        <span>₹{(applyDiscountsPerUnit(it, it.quantity) * it.quantity).toFixed(2)}</span>
                      </div>
                    ))}
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6">
          {cart.length === 0 ? (
            ...
            <div className="space-y-4">
              {cart.map(item => {
                const discountedPrice = applyDiscountsPerUnit(item, item.quantity);
                return (
  ...
  if (selectedProduct) {
    const discountedPrice = applyDiscountsPerUnit(selectedProduct, selectedProduct.quantity || 1);
    return (
      <div className="h-screen bg-gray-50 flex flex-col relative">
            ...
            <div className="mb-4">
              ...
              {selectedProduct.combo_quantity > 0 && (
                <div className="bg-orange-50 border border-orange-200 rounded-lg p-3 mb-4">
                  ...
                  <p className="text-orange-700 text-sm">Buy {selectedProduct.combo_quantity} or more, get {selectedProduct.combo_discount}% extra discount (applied on top of item discount).</p>
                </div>
              )}
            </div>
            <div className="mb-6">
              ...
              <div className="mt-3 text-sm text-gray-700">
                <div><strong>Store:</strong> {shopData.BusinessName} ({shopData.BusinessType})</div>
                <div><strong>Address:</strong> {shopData.Address}, {shopData.Pincode}</div>
                <div><strong>Phone:</strong> {shopData.MobileNo}</div>
              </div>
            </div>
  ...
  return (
    <div className="h-screen bg-gray-50 flex flex-col relative">
      ...
      <div className="bg-gradient-to-r from-blue-600 to-blue-400 px-6 pt-4 pb-4">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-3">
            <h1 className="text-white text-2xl font-bold">{shopData.BusinessName}</h1>
            <div>
              <button onClick={() => setShowShopInfoModal(true)} className="ml-2 bg-white bg-opacity-10 hover:bg-opacity-20 p-1 rounded-full text-white" aria-label="Shop info" title="Shop info">
                <Info size={16} />
              </button>
            </div>
          </div>
      ...
      {/* Centered Shop Info Modal */}
      {showShopInfoModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40 p-4">
          <div className="bg-white rounded-2xl max-w-lg w-full shadow-2xl relative">
            <button onClick={() => setShowShopInfoModal(false)} className="absolute right-3 top-3 text-gray-600">
              <X size={18} />
            </button>
            <div className="p-6">
              ...
              <div className="text-sm text-gray-700 mb-1"><strong>Pincode:</strong> {shopData.Pincode}</div>
              <div className="mt-3">
                <a href={shopData.MapLocation} target="_blank" rel="noopener noreferrer" className="text-blue-600 underline">Open on map</a>
              </div>
            </div>
          </div>
        </div>
        ...
        {filteredProducts.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {filteredProducts.map((product: any) => {
              const discountedPrice = applyDiscountsPerUnit(product, 1);
              return (
                      ...
                      <div className="mb-2">
                        ...
                        <div className="text-xs text-gray-500 mb-2">
                          ...
                          <div className="text-xs text-gray-400 mt-1">
                            <span>{shopData.BusinessType} • {shopData.Address}</span>
                          </div>
                        </div>
                      ...
                      <div className="mt-auto">
                        ...
                        {product.combo_quantity > 0 && (
                          <div className="bg-orange-50 border border-orange-200 rounded-lg p-2 mb-3">
                            <p className="text-orange-800 font-semibold text-xs">🎉 Buy {product.combo_quantity}+ get {product.combo_discount}% extra off (on top of item discount)</p>
                          </div>
                        )}
